...
function fechaHora($fecha) {
	...
	$fechaeuropean = trim ( $dia ) . "/" . trim ( $mes ) . "/" . trim ( $ano ).' '.$fechaAux[1];	
	return $fechaeuropean;
}
...
/**
 * Retorna la edad dado su fecha de nacimiento
 */
...
//     $tam = strlen($Y);
//     if ($tam==3) {
//     	$fecha = fechaLatina($fecha);
//     	list($d,$m,$Y) = explode("/",$fecha);
//     }
//     return( date("md") < $m.$d ? date("Y")-$Y-1 : date("Y")-$Y );
}
//function CalculaEdadDetallada( $fecha ) {
//	if ($fecha == null || $fecha == "") {
//		return null;
//	}
//	//	ECHO "--$fecha--";
...
function interval2double($interval) {
	...
	list ( $years, $tagyears, $months, $tagmonths, $days, $tagdays ) = explode ( " ", $interval );
